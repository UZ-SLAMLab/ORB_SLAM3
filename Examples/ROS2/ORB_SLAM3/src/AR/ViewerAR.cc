...
Plane* ViewerAR::DetectPlane(const cv::Mat Tcw, const std::vector<MapPoint*> &vMPs, const int iterations)
{
    ...
    for(size_t i=0; i<vMPs.size(); i++)
    {
        ...
        if(pMP)
        {
            ...
            {
                cv::Mat worldPos(3, 1, CV_32F);
                worldPos.at<float>(0) = pMP->GetWorldPos()(0);
                worldPos.at<float>(1) = pMP->GetWorldPos()(1);
                worldPos.at<float>(2) = pMP->GetWorldPos()(2);
...
void Plane::Recompute()
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(!pMP->isBad())
        {
            ...
            Xw.at<float>(0) = pMP->GetWorldPos()(0);
            Xw.at<float>(1) = pMP->GetWorldPos()(1);
            Xw.at<float>(2) = pMP->GetWorldPos()(2);
            o+=Xw;
