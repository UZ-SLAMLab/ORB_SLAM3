...
//static
vector<Plane*> ViewerAR::vpPlane;

ViewerAR::ViewerAR(){}

void ViewerAR::CreatePanel(int w, int h)
{
    ...
    menu_detectplane = std::unique_ptr<pangolin::Var<bool>>(new pangolin::Var<bool>("menu.Insert Cube",false,false));
    menu_clear = std::unique_ptr<pangolin::Var<bool>>(new pangolin::Var<bool>("menu.Clear All",false,false));
    menuReset = std::unique_ptr<pangolin::Var<bool>>(new pangolin::Var<bool>("menu.Reset",false,false));

    menu_drawim = std::unique_ptr<pangolin::Var<bool>>(new pangolin::Var<bool>("menu.Draw Image",true,true));
    menu_drawcube = std::unique_ptr<pangolin::Var<bool>>(new pangolin::Var<bool>("menu.Draw Cube",true,true));
    menu_cubesize = std::unique_ptr<pangolin::Var<float>>(new pangolin::Var<float>("menu. Cube Size",0.05,0.01,0.3));
    menu_drawgrid = std::unique_ptr<pangolin::Var<bool>>(new pangolin::Var<bool>("menu.Draw Grid",false,true));
    menu_ngrid = std::unique_ptr<pangolin::Var<int>>(new pangolin::Var<int>("menu. Grid Elements",3,1,10));
    menu_sizegrid = std::unique_ptr<pangolin::Var<float>>(new pangolin::Var<float>("menu. Element Size",0.05,0.01,0.3));
    menu_drawpoints = std::unique_ptr<pangolin::Var<bool>>(new pangolin::Var<bool>("menu.Draw Points",true,true));

    menu_LocalizationMode = std::unique_ptr<pangolin::Var<bool>>(new pangolin::Var<bool>("menu.Localization Mode",false,true));


    _projMatrix = pangolin::ProjectionMatrix(w,h,fx,fy,cx,cy,0.01,100);
    s_cam.SetProjectionMatrix(_projMatrix);
    projectionMatrixRDF = pangolin::ProjectionMatrixRDF_TopLeft(w,h,fx,fy,cx,cy,0.01,100);
    // s_cam.Set(projectionMatrixRDF);
    // _projMatrix = proj;//s_cam.GetProjectionMatrix();

    // handler = new ORB_SLAM3::ViewHandler3D(s_cam);
    ...
    mvImageTexture = new pangolin::GlTexture(w,h,GL_RGB,false,0,GL_RGB,GL_UNSIGNED_BYTE);

    _viewport = d_image.GetBounds();
}

void ViewerAR::Refresh()
{
    cv::Mat im, Tcw;
    int status;
    ...
    // std::lock_guard<std::mutex> mtx_locker(mtx);//同步
    // _isTaskLoopWait = handler->IsSelectingMapPoints();
    ...
    // Draw image
    if(*menu_drawim)
        DrawImageTexture(*mvImageTexture, im);

    glClear(GL_DEPTH_BUFFER_BIT);

    // Load camera projection
    glMatrixMode(GL_PROJECTION);
    projectionMatrixRDF.Load();
