...
void Optimizer::InertialOptimization(Map *pMap, Eigen::Matrix3d &Rwg, double &scale, Eigen::Vector3d &bg, Eigen::Vector3d &ba, Eigen::Matrix4d &Tbc, bool bMono, Eigen::MatrixXd  &covInertial, bool bFixedVel, bool bGauss, float priorG, float priorA)
{
    Verbose::PrintMess("inertial optimization", Verbose::VERBOSITY_NORMAL);
    int its = 200;
    long unsigned int maxKFid = pMap->GetMaxKFid();
    ...
    for(size_t i=0;i<vpKFs.size();i++)
    {
        ...
        if(pKFi->mnId<=maxKFid)
        {
            ...
            ei->setVertex(1,dynamic_cast<g2o::OptimizableGraph::Vertex*>(VTbc));
