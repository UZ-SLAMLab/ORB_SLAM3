...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, std::vector<KeyFrame*> &vpNonEnoughOptKFs)
{
    ...
    for(std::list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        for(std::map<KeyFrame*,std::tuple<int,int>>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
            {
                const int cam0Index = std::get<0>(mit->second);
...
void Optimizer::OptimizeEssentialGraph6DoF(KeyFrame* pCurKF, std::vector<KeyFrame*> &vpFixedKFs, std::vector<KeyFrame*> &vpFixedCorrectedKFs,
                                       std::vector<KeyFrame*> &vpNonFixedKFs, std::vector<MapPoint*> &vpNonCorrectedMPs, double scale)
{
    ...
    std::vector<g2o::SE3Quat,Eigen::aligned_allocator<g2o::SE3Quat> > vScw(nMaxKFid+1);
    std::vector<g2o::SE3Quat,Eigen::aligned_allocator<g2o::SE3Quat> > vScw_bef(nMaxKFid+1);
    std::vector<g2o::SE3Quat,Eigen::aligned_allocator<g2o::SE3Quat> > vCorrectedSwc(nMaxKFid+1);
    std::vector<g2o::VertexSE3Expmap*> vpVertices(nMaxKFid+1);
    std::vector<bool> vbFromOtherMap(nMaxKFid+1);
                                       ...
                                       const LoopClosing::KeyFrameAndPose &CorrectedSim3)
{
    ...
    for(size_t i=0, iend=vpKFs.size(); i<iend;i++)
    {
        ...
        if(pKF->mnBALocalForKF==pCurKF->mnId || pKF->mnBAFixedForKF==pCurKF->mnId){
            std::cout << "fixed fk: " << pKF->mnId << std::endl;
            VSim3->setFixed(true);
        }
        else
            VSim3->setFixed(false);
...
int Optimizer::OptimizeSim3(KeyFrame *pKF1, KeyFrame *pKF2, std::vector<MapPoint *> &vpMatches1, g2o::Sim3 &g2oS12, const float th2,
...
int Optimizer::OptimizeSim3(KeyFrame *pKF1, KeyFrame *pKF2, std::vector<MapPoint *> &vpMatches1, std::vector<KeyFrame*> &vpMatches1KF, g2o::Sim3 &g2oS12, const float th2,
                            const bool bFixScale, Eigen::Matrix<double,7,7> &mAcumHessian, const bool bAllPoints)
{
    g2o::SparseOptimizer optimizer;
    // g2o::BlockSolverX::LinearSolverType * linearSolver;
    ...
    for(int i=0; i<N; i++)
    {
        ...
        pKFm = vpMatches1KF[i];
        const int i2 = std::get<0>(pMP2->GetIndexInKeyFrame(pKFm));
        if(i2 < 0)
...
void Optimizer::LocalInertialBA(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges, bool bLarge, bool bRecInit)
{
    ...
    //cout << "Total std::map points: " << lLocalMapPoints.size() << std::endl;
    ...
    Verbose::PrintMess("LIBA KFs: " + std::to_string(N), Verbose::VERBOSITY_DEBUG);
    Verbose::PrintMess("LIBA bNonFixed?: " + std::to_string(bNonFixed), Verbose::VERBOSITY_DEBUG);
    Verbose::PrintMess("LIBA KFs visual outliers: " + std::to_string(vToErase.size()), Verbose::VERBOSITY_DEBUG);
...
void Optimizer::InertialOptimization(Map *pMap, Eigen::Vector3d &bg, Eigen::Vector3d &ba, float priorG, float priorA)
{
    ...
    VS->setId(maxKFid*2+5);
    VS->setFixed(true); // Fixed since scale is obtained from already well initialized std::map
    optimizer.addVertex(VS);
...
void Optimizer::InertialOptimization(std::vector<KeyFrame*> vpKFs, Eigen::Vector3d &bg, Eigen::Vector3d &ba, float priorG, float priorA)
{
    int its = 200;
...
void Optimizer::MergeBundleAdjustmentVisual(KeyFrame* pCurrentKF, std::vector<KeyFrame*> vpWeldingKFs, std::vector<KeyFrame*> vpFixedKFs, bool *pbStopFlag)
{
    ...
    for(unsigned int i=0; i < vpMPs.size(); ++i)
    {
        ...
        for(std::map<KeyFrame*,std::tuple<int,int>>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            KeyFrame* pKF = mit->first;
            if(spKeyFrameBA.find(pKF) == spKeyFrameBA.end() || pKF->isBad() || pKF->mnId>maxKFid || pKF->mnBALocalForKF != pCurrentKF->mnId || !pKF->GetMapPoint(std::get<0>(mit->second)))
                continue;
    ...
    // Get Map Mutex
    std::unique_lock<std::mutex> lock(pCurrentKF->GetMap()->mMutexMapUpdate);
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pMainKF,std::vector<KeyFrame*> vpAdjustKF, std::vector<KeyFrame*> vpFixedKF, bool *pbStopFlag)
{
    ...
    for(KeyFrame* pKFi : vpFixedKF)
    {
        if(pKFi->isBad() || pKFi->GetMap() != pCurrentMap)
        {
            Verbose::PrintMess("ERROR LBA: KF is bad or is not in the current std::map", Verbose::VERBOSITY_NORMAL);
            continue;
        }
    ...
    std::map<int, int> mStatsObs;
    for(std::map<MapPoint*, int>::iterator it = mpObsMPs.begin(); it != mpObsMPs.end(); ++it)
    {
    ...
    if(bDoMore)
    {
        ...
        Verbose::PrintMess("LBA: First optimization, there are " + std::to_string(badMonoMP) + " monocular and " + std::to_string(badStereoMP) + " sterero bad edges", Verbose::VERBOSITY_DEBUG);
    ...
    Verbose::PrintMess("LBA: Second optimization, there are " + std::to_string(badMonoMP) + " monocular and " + std::to_string(badStereoMP) + " sterero bad edges", Verbose::VERBOSITY_DEBUG);
    ...
    if(!vToErase.empty())
    {
        std::map<KeyFrame*, int> mpMPs_in_KF;
        ...
        Verbose::PrintMess("LBA: There are " + std::to_string(vToErase.size()) + " observations whose will be deleted from the std::map", Verbose::VERBOSITY_DEBUG);
        for(size_t i=0;i<vToErase.size();i++)
        {
            KeyFrame* pKFi = vToErase[i].first;
            MapPoint* pMPi = vToErase[i].second;
        ...
        Verbose::PrintMess("LBA: " + std::to_string(spErasedMPs.size()) + " MPs had deleted observations", Verbose::VERBOSITY_DEBUG);
        Verbose::PrintMess("LBA: Current std::map is " + std::to_string(pMainKF->GetMap()->GetId()), Verbose::VERBOSITY_DEBUG);
        int numErasedMP = 0;
        for(MapPoint* pMPi : spErasedMPs)
        {
            if(pMPi->isBad())
            {
                Verbose::PrintMess("LBA: MP " + std::to_string(pMPi->mnId) + " has lost almost all the observations, its origin std::map is " + std::to_string(pMPi->mnOriginMapId), Verbose::VERBOSITY_DEBUG);
                numErasedMP++;
            }
        }
        Verbose::PrintMess("LBA: " + std::to_string(numErasedMP) + " MPs had deleted from the std::map", Verbose::VERBOSITY_DEBUG);

        for(KeyFrame* pKFi : spErasedKFs)
        {
            int num_MPs = pKFi->GetMapPoints().size();
            int num_init_MPs = mpMPs_in_KF[pKFi];
            Verbose::PrintMess("LBA: Initially KF " + std::to_string(pKFi->mnId) + " had " + std::to_string(num_init_MPs) + ", at the end has " + std::to_string(num_MPs), Verbose::VERBOSITY_DEBUG);
        }
    }
    for(unsigned int i=0; i < vpMPs.size(); ++i)
    {
    ...
    for(KeyFrame* pKFi : vpAdjustKF)
    {
        ...
        {
            Verbose::PrintMess("LBA ERROR: KF " + std::to_string(pKFi->mnId) + " has only " + std::to_string(numMonoOptPoints) + " monocular and " + std::to_string(numStereoOptPoints) + " stereo points", Verbose::VERBOSITY_DEBUG);
        }
...
void Optimizer::MergeInertialBA(KeyFrame* pCurrKF, KeyFrame* pMergeKF, bool *pbStopFlag, Map *pMap, LoopClosing::KeyFrameAndPose &corrPoses)
{
    ...
    std::vector<KeyFrame*> vNeighCurr = pCurrKF->GetCovisiblesByWeight(min_obs);
    for(std::vector<KeyFrame*>::iterator lit=vNeighCurr.begin(), lend=vNeighCurr.end(); lit!=lend; lit++)
    {
        if(i>=maxCovKF)
            break;
        KeyFrame* pKFi = *lit;
    ...
    i = 0;
    std::vector<KeyFrame*> vNeighMerge = pMergeKF->GetCovisiblesByWeight(min_obs);
    for(std::vector<KeyFrame*>::iterator lit=vNeighCurr.begin(), lend=vNeighCurr.end(); lit!=lend; lit++, i++)
    {
        if(i>=maxCovKF)
            break;
        KeyFrame* pKFi = *lit;
    ...
    for(int i=0;i<N;i++)
    {
        //cout << "inserting inertial edge " << i << std::endl;
        KeyFrame* pKFi = vpOptimizableKFs[i];
