...
namespace ORB_SLAM3
{
    ...
    System::System()
    {
        std::cout << "System empty constructor" << std::endl;
    ...
    void System::System_Initialize(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
    ...
    void System::Shutdown()
    {
        ...
        // ======================================================================================================
        // opened these parts to prevent segmentation fault at viewer-> "pangolin::FinishFrame();"
        if (mpViewer)
        {
    ...
    void System::SaveTrajectoryEuRoC(const string &filename)
    {
             ...
             lit != lend; lit++, lRit++, lT++, lbL++)
        {
            ...
            if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor == IMU_RGBD)
            {
                ...
                f << setprecision(6) << 1e6 * (*lT) << " " << setprecision(6) << twb(0) << " " << twb(1) << " " << twb(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
            }
            else
            {
                ...
                f << setprecision(6) << 1e6 * (*lT) << " " << setprecision(6) << twc(0) << " " << twc(1) << " " << twc(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
            }
    ...
    void System::SaveKeyFrameTrajectoryEuRoC(const string &filename)
    {
        ...
        std::cout << "map size in atlas: " << vpMaps.size() << std::endl;

        Map *pBiggerMap = nullptr;
        int numMaxKFs = 0;
        for (Map *pMap : vpMaps)
        {
            std::cout << "all keyframe size for map : " << pMap->GetAllKeyFrames().size() << std::endl;
        ...
        vector<KeyFrame *> vpKFs = pBiggerMap->GetAllKeyFrames();
        sort(vpKFs.begin(), vpKFs.end(), KeyFrame::lId);
        std::cout << "sorted keyframes" << std::endl;
        ...
        for (size_t i = 0; i < vpKFs.size(); i++)
        {
            ...
            if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor == IMU_RGBD)
            {
                ...
                f << setprecision(6) << 1e6 * pKF->mTimeStamp << " " << setprecision(6) << twb(0) << " " << twb(1) << " " << twb(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
            }
            else
            {
                Sophus::SE3f Twc = pKF->GetPoseInverse();
                ...
                f << setprecision(6) << 1e6 * pKF->mTimeStamp << " " << setprecision(6) << t(0) << " " << t(1) << " " << t(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
            }
        }
        f.close();
        cout << endl
             << "End of saving keyframes to " << filename << " ..." << endl;
    }
    ...
    void System::SaveAtlas(int type)
    {
        if (!mStrSaveAtlasToFile.empty())
        {
            ...
            std::cout << "System::SaveAtlas : " << mStrSaveAtlasToFile << std::endl;
        ...
        {
            std::cout << "System::SaveAtlas file string empty !!!" << std::endl;
        }
    }
    ...
    void System::SetVocabulary(ORBVocabulary *ptrVocabulary)
    {
        // mpVocabulary = ptrVocabulary.get();
        mpVocabulary = ptrVocabulary;
    }
    void System::SetKeyFrameDB(KeyFrameDatabase *ptrKeyFrameDB)
    {
        // mpKeyFrameDatabase = ptrKeyFrameDB.get();
        mpKeyFrameDatabase = ptrKeyFrameDB;
    }
    void System::SetAtlas(Atlas *ptrAtlas)
    {
        // mpAtlas = ptrAtlas.get();
        mpAtlas = ptrAtlas;
    }
    void System::SetFrameDrawer(FrameDrawer *ptrFrameDrawer)
    {
        // mpFrameDrawer = ptrFrameDrawer.get();
        mpFrameDrawer = ptrFrameDrawer;
    }
    void System::SetMapDrawer(MapDrawer *ptrMapDrawer)
    {
        // mpMapDrawer = ptrMapDrawer.get();
        mpMapDrawer = ptrMapDrawer;
    }
    void System::SetTracking(Tracking *ptrTracking)
    {
        // mpTracker = ptrTracking.get();
        mpTracker = ptrTracking;
    }
    void System::SetLocalMapping(LocalMapping *ptrLocalMapping)
    {
        // mpLocalMapper = ptrLocalMapping.get();
        mpLocalMapper = ptrLocalMapping;
    }
    void System::SetLoopClosing(LoopClosing *ptrLoopClosing)
    {
        // mpLoopCloser = ptrLoopClosing.get();
        mpLoopCloser = ptrLoopClosing;
    }
    void System::SetViewer(Viewer *ptrViewer)
    {
        // mpViewer = ptrViewer.get();
        mpViewer = ptrViewer;
    }
    ...
    KeyFrameDatabase &System::GetKeyFrameDB()
    {
    ...
    void System::CreateKeyFrameDB(const ORBVocabulary &voc)
    {
        mpKeyFrameDatabase = new ORB_SLAM3::KeyFrameDatabase(voc);
    }
    void System::CreateAtlas(int initKFid)
    {
        mpAtlas = new ORB_SLAM3::Atlas(initKFid);
    }
    void System::CreateAtlas()
    {
        mpAtlas = new ORB_SLAM3::Atlas();
    }
    void System::CreateFrameDrawer(Atlas *pAtlas)
    {
        mpFrameDrawer = new ORB_SLAM3::FrameDrawer(pAtlas);
    }
    void System::CreateMapDrawer(Atlas *pAtlas, const string &strSettingPath, Settings *settings)
    {
        mpMapDrawer = new ORB_SLAM3::MapDrawer(pAtlas, strSettingPath, settings);
    }
    void System::CreateTracking(System *pSys, ORBVocabulary *pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Atlas *pAtlas, KeyFrameDatabase *pKFDB, const string &strSettingPath, const int sensor, Settings *settings, const string &_nameSeq)
    {
        mpTracker = new ORB_SLAM3::Tracking(pSys, pVoc, pFrameDrawer, pMapDrawer,
                                            pAtlas, pKFDB, strSettingPath, sensor, settings, _nameSeq);
    }
    void System::CreateLocalMapping(System *pSys, Atlas *pAtlas, const float bMonocular, bool bInertial, const string &_strSeqName)
    {
        mpLocalMapper = new ORB_SLAM3::LocalMapping(pSys, pAtlas, bMonocular, bInertial, _strSeqName);
    }
    void System::CreateLoopClosing(Atlas *pAtlas, KeyFrameDatabase *pDB, ORBVocabulary *pVoc, const bool bFixScale, const bool bActiveLC)
    {
        mpLoopCloser = new ORB_SLAM3::LoopClosing(pAtlas, pDB, pVoc, bFixScale, bActiveLC);
    }
    void System::CreateViewer(System *pSystem, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Tracking *pTracking, const string &strSettingPath, Settings *settings)
    {
        mpViewer = new ORB_SLAM3::Viewer(pSystem, pFrameDrawer, pMapDrawer, pTracking, strSettingPath, settings);
    }

    void System::SaveAtlasViaShutdown(const std::string mStrSaveAtlasToFile)
    {
        std::cout << "SaveAtlasViaShutdown : " << mStrSaveAtlasToFile << std::endl;
        std::cout << "-- input empty? : " << mStrSaveAtlasToFile.empty() << " map size : " << mpAtlas->GetAllMaps().size() << std::endl;
        if (!mStrSaveAtlasToFile.empty() && mpAtlas->GetAllMaps().size() != 0)
        {
            Verbose::PrintMess("Atlas saving to file " + mStrSaveAtlasToFile, Verbose::VERBOSITY_NORMAL);
            System::SaveAtlas(FileType::BINARY_FILE);
        }
        else
        {
            std::cout << "received string empty, cant save !" << std::endl;
        }
    }

    std::pair<bool, bool> System::getLocalizationModeStates()
    {
        unique_lock<mutex> lock(mMutexMode);
