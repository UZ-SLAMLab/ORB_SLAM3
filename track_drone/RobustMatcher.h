...
class RobustMatcher {
 public:
  RobustMatcher() : detector_(), extractor_(), matcher_(),
					ratio_(0.2f), training_img_(), img_matching_() {
  ...
  [[nodiscard]] cv::Mat getImageMatching() const { return img_matching_; }

  // Set ratio parameter for the ratio test
  void setRatio(float rat) { ratio_ = rat; }

  [[maybe_unused]] void setTrainingImage(const cv::Mat &img) { training_img_ = img; }
  ...
  int ratioTest2(std::vector<std::vector<cv::DMatch> > &matches) const;

  // Insert symmetrical matches in symMatches vector
  static void symmetryTest(const std::vector<std::vector<cv::DMatch> > &matches1,
					const std::vector<std::vector<cv::DMatch> > &matches2,
					std::vector<cv::DMatch> &symMatches);

  // Match feature points using ratio and symmetry test
  [[maybe_unused]] void robustMatch(const cv::Mat &frame, std::vector<cv::DMatch> &good_matches,
  ...
  void calcdesriptorsperframe(const cv::Mat &frame,
							  std::vector<cv::KeyPoint> &keypoints_frame,
							  cv::Mat &descriptors_frame);
  void fastRobustMatch2(std::vector<cv::DMatch> &good_matches,
						cv::Mat &descriptors_frame,
						const cv::Mat &descriptors_model);
  [[maybe_unused]] void fastRobustMatch3(std::vector<cv::DMatch> &good_matches,
						cv::Mat &descriptors_frame,
						const cv::Mat &descriptors_model);
  [[maybe_unused]] void fastRobustMatchmap(std::vector<cv::DMatch> &good_matches,
						  std::vector<cv::KeyPoint> &keypoints_frame,
