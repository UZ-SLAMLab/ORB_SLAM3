...
namespace PnP {
    ...
    public:
        ...
        PnpProblemSolver(const SparseRowMatrix &A_rows,
        ...
        PnpResult solve_pnp(std::shared_ptr<PnpObjective> pnp_objective,
                            std::shared_ptr<BarrierMethodSettings> barrier_method_settings);

        void line_search(const ColVector<Y_SIZE> &delta_y,
                         const std::function<double(const ColVector<Y_SIZE> &)> &obj_func,
                         std::shared_ptr<BarrierMethodSettings> barrier_settings);

        void centering_step(const std::function<double(const ColVector<Y_SIZE> &)>& obj_func,
        ...
        Eigen::Vector4d perform_local_search(std::shared_ptr<PnpObjective> pnp_objective, Eigen::Vector4d quats);
    };
}
