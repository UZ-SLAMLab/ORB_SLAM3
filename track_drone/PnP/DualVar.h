...
namespace PnP
{
	...
	class DualVar
	{
	 public:
		...
		Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> hessian_multiplication_mat;

		DualVar(ColVector<Y_SIZE> y_vec,
			ColVector<NUM_CONSTRAINTS> zero_vars,
			ColVector<A_ROWS> slack,
			ColMatrix<M_MATRIX_DIM, M_MATRIX_DIM> matrix_15_15,
			ColMatrix<M_MATRIX_DIM, M_MATRIX_DIM> matrix_15_15_inv,
		...
		static std::shared_ptr<DualVar> init(ColVector<Y_SIZE> y_vec,
			const SparseRowMatrix& A_mat_rows,
			const RowMatrix<NUM_CONSTRAINTS, Y_SIZE>& A_sub_rows,
			const SparseColMatrix& c_vec);

		void set_y_vec(ColVector<Y_SIZE> y_vec, const SparseRowMatrix& A_mat_rows, const SparseColMatrix& c_vec);
