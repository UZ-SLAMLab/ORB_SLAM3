...
namespace PnP
{
	std::shared_ptr<PnpInput> PnpInput::parse_input(int argc, char** argv)
	{
		if (argc != 5)
		{
			std::cout << "Expected 4 command line arguments:" << std::endl;
		...
		{
			std::cout << "points amount = " << points.size() << " whereas lines amount = " << lines.size() << std::endl;
			exit(1);
		}

		if (indices.size() != weights.size())
		{
			std::cout << "weights amount = " << weights.size() << " whereas indices amount = " << indices.size()
					  << std::endl;
			exit(1);
		}
	...
	PnpInput::PnpInput(std::vector<Eigen::Vector3d> points, std::vector<Eigen::Vector3d> lines,
		std::vector<double> weights, std::vector<int> indices, int indices_amount)
		: points(std::move(points)), lines(std::move(lines)), weights(std::move(weights)), indices(std::move(indices)),
		  indices_amount(indices_amount)
	{
	}
	...
	{
		return std::make_shared<PnpInput>(std::move(points),
			std::move(lines),
			std::move(weights),
			std::move(indices),
			indices.size());
