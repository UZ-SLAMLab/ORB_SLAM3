...
namespace PnP {
    ...
    double calc_cost(Eigen::Vector4d quats, std::shared_ptr<PnpObjective> objective){
        auto q1 = quats.x();
        auto q2 = quats.y();
        auto q3 = quats.z();
        auto q4 = quats.w();
    ...
    void PnpProblemSolver::line_search(const ColVector<Y_SIZE>& delta_y,
        ...
        Eigen::Map<ColMatrix<M_MATRIX_DIM, M_MATRIX_DIM>> B_15_15(dual_var->B_vec.data() + NUM_CONSTRAINTS, M_MATRIX_DIM, M_MATRIX_DIM);

        //auto L_15 = dual_var->matrix_15_15.llt().matrixL().toDenseMatrix().inverse().eval();
        //auto eigs = (L_15 * B_15_15 * L_15.transpose()).selfadjointView<Eigen::Upper>().eigenvalues().eval();

        double eigs_pow_minus1[M_MATRIX_DIM];
        Eigen::LLT<Eigen::Ref<ColMatrix<M_MATRIX_DIM, M_MATRIX_DIM>>> llt_of_matrix_15_15(dual_var->matrix_15_15);
        dual_var->temp_col_1.noalias() = llt_of_matrix_15_15.matrixL().toDenseMatrix();
        dual_var->temp_row_2.noalias() = dual_var->temp_col_1.inverse(); // temp_row_2 = L^-1
        dual_var->temp_row_1.noalias() = dual_var->temp_row_2 * B_15_15;
        dual_var->temp_col_2.noalias() = dual_var->temp_row_1 * dual_var->temp_row_2.transpose();
        dual_var->temp_vec.noalias()   = dual_var->temp_col_2.selfadjointView<Eigen::Upper>().eigenvalues();
        auto& eigs = dual_var->temp_vec;
        for (int i = 0; i < M_MATRIX_DIM; i++)
            eigs_pow_minus1[i] = 1.0 / eigs(i, 0);
        auto const_val = obj_func(delta_y);
        std::function<double(double)> line_search_gradient = [const_val, &eigs_pow_minus1](double t) {
        ...
        auto result = find_zero_bin_search(line_search_gradient, 0.0, min_val, barrier_settings->binary_search_depth);
        dual_var->set_y_vec(dual_var->y_vec + result * delta_y, A_rows, c_vec);
    }

    void PnpProblemSolver::centering_step(const std::function<double(const ColVector<Y_SIZE>&)>& obj_func,
        ...
        while(true){
            ...
            if ((inner_iteration >= barrier_method_settings->max_inner_iterations) ||
                ...
                current_result = PnpResult(this->perform_local_search(pnp_objective, current_quats), pnp_objective);
                if (current_result.cost() < pnp_result.cost())
                    pnp_result = current_result;
                break;
            }
        ...
        if(barrier_method_settings->verbose)
            std::cout << outer_iteration << ") " << inner_iteration << " iterations" << std::endl;
    }


    PnpResult PnpProblemSolver::solve_pnp (std::shared_ptr<PnpObjective> pnp_objective,
                                           std::shared_ptr<BarrierMethodSettings> barrier_method_settings){
        double t = 100; // 1/t is the barrier weight
        ...
        std::function<double(const ColVector<Y_SIZE>&)> obj_func = [&pnp_objective, &t](const ColVector<Y_SIZE> &y_vec) -> double {
            return -t * pnp_objective->b.tail<35>().dot(y_vec.tail<35>()); // first 34 values of b are zeros
        };
        std::function<const Eigen::VectorXd&(DualVar&)> calc_grad = [this, &gradient, &pnp_objective, &t](DualVar &y) -> const Eigen::VectorXd&{
            gradient.noalias() = -t * pnp_objective->b.transpose() + (*(y.cone_barrier_gradient(*this)));
            return gradient;
        };
        std::function<const Eigen::MatrixXd&(DualVar&)> calc_hess = [this, &pnp_objective, &t](DualVar &y) -> const Eigen::MatrixXd& {
        ...
        for (int outer_iteration = 1; !is_last_iter; outer_iteration++) {
            is_last_iter = t * barrier_method_settings->miu > 1.0 / barrier_method_settings->epsilon;
            this->centering_step(obj_func, calc_grad, calc_hess, outer_iteration, is_last_iter, pnp_result, pnp_objective, barrier_method_settings);
