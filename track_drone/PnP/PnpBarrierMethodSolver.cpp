...
namespace PnP
{
		...
		std::shared_ptr<BarrierMethodSettings> barrier_settings)
	{
		...
		Eigen::Map<ColMatrix<M_MATRIX_DIM, M_MATRIX_DIM>>
			B_15_15(dual_var->B_vec.data() + NUM_CONSTRAINTS, M_MATRIX_DIM, M_MATRIX_DIM);
		...
		std::shared_ptr<BarrierMethodSettings> barrier_method_settings)
	{
		...
		std::function<double(const ColVector<Y_SIZE>&)>
			obj_func = [&pnp_objective, &t](const ColVector<Y_SIZE>& y_vec) -> double
		{
		  return -t * pnp_objective->b.tail<35>().dot(y_vec.tail<35>()); // first 34 values of b are zeros
		};
		std::function<const Eigen::VectorXd&(DualVar&)>
			calc_grad = [this, &gradient, &pnp_objective, &t](DualVar& y) -> const Eigen::VectorXd&
		{
		  gradient.noalias() = -t * pnp_objective->b.transpose() + (*(y.cone_barrier_gradient(*this)));
		  return gradient;
		};
		std::function<const Eigen::MatrixXd&(DualVar&)>
			calc_hess = [this, &pnp_objective, &t](DualVar& y) -> const Eigen::MatrixXd&
		{
