...
namespace PnP
{
		  ...
		  eigenvals(std::move(eigenvals))
	{
		...
		this->hessian_multiplication_mat =
			Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>(Y_SIZE, A_ROWS);
	...
	Eigen::MatrixXd* DualVar::cone_barrier_hessian(const PnpProblemSolver& pnp)
	{
		...
		hessian_helper_mat.block(NUM_CONSTRAINTS, NUM_CONSTRAINTS, A_ROWS - NUM_CONSTRAINTS, A_ROWS - NUM_CONSTRAINTS)
			.noalias() =
...
//		auto cop = hessian_helper_mat.eval();
		...
		for (int i = 0; i < M_MATRIX_DIM; i++)
			for (int j = 0; j < M_MATRIX_DIM; j++)
				if (i != j)
					hessian_helper_mat
						.block(M_MATRIX_DIM + i * M_MATRIX_DIM, M_MATRIX_DIM + j * M_MATRIX_DIM, M_MATRIX_DIM,
		...
		const SparseColMatrix& c_vec)
	{
		...
		Eigen::Map<ColMatrix<M_MATRIX_DIM, M_MATRIX_DIM>>
			matrix_15_15(slack.data() + NUM_CONSTRAINTS, M_MATRIX_DIM, M_MATRIX_DIM);
		for (int i = 0; i < M_MATRIX_DIM; i++)
		...
		return std::make_shared<DualVar>(std::move(y_vec),
			zero_part,
			slack,
			matrix_15_15,
			matrix_15_15_inv,
			matrix_15_15_inv_vec,
			eigenvals,
			A_sub_rows);
	}
		...
		const SparseColMatrix& c_vec)
	{
		...
		Eigen::Map<ColMatrix<M_MATRIX_DIM, M_MATRIX_DIM>>
			new_matrix_15_15(slack.data() + NUM_CONSTRAINTS, M_MATRIX_DIM, M_MATRIX_DIM);
		for (int i = 0; i < M_MATRIX_DIM; i++)
		{
