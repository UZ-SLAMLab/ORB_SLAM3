...
namespace PnP {
    ...
    DualVar::DualVar(ColVector<Y_SIZE> y_vec,
            ...
            : y_vec(std::move(y_vec)),
              zero_vars(std::move(zero_vars)),
              slack(std::move(slack)),
              matrix_15_15(std::move(matrix_15_15)),
        ...
        this->hessian_multiplication_mat = Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>(Y_SIZE, A_ROWS);
        ...
        this->equation_mat = Eigen::MatrixXd::Zero(Y_SIZE + NUM_CONSTRAINTS,Y_SIZE + NUM_CONSTRAINTS);
        this->equation_mat.block(0,Y_SIZE,Y_SIZE,NUM_CONSTRAINTS) = A_sub_rows.transpose();
    ...
    Eigen::VectorXd* DualVar::cone_barrier_gradient(const PnpProblemSolver &pnp) {
        ...
        gradient_helper_vec.block<M_MATRIX_DIM * M_MATRIX_DIM, 1>(NUM_CONSTRAINTS, 0) = this->matrix_15_15_inv_vec;
    ...
    Eigen::MatrixXd* DualVar::cone_barrier_hessian(const PnpProblemSolver &pnp) {
       ...
       // hessian_helper_mat.block(0, 0, NUM_CONSTRAINTS, A_ROWS).setZero();
        ...
        hessian_helper_mat.block(NUM_CONSTRAINTS, NUM_CONSTRAINTS, A_ROWS - NUM_CONSTRAINTS, A_ROWS - NUM_CONSTRAINTS).noalias() =
        ...
        for (int i = 0; i < M_MATRIX_DIM; i++)
            for (int j = 0; j < M_MATRIX_DIM; j++)
                if (i != j)
                    hessian_helper_mat.block(M_MATRIX_DIM + i * M_MATRIX_DIM, M_MATRIX_DIM + j * M_MATRIX_DIM, M_MATRIX_DIM,
                              M_MATRIX_DIM).transposeInPlace();

        hessian_multiplication_mat.noalias() = pnp.A_cols.transpose() * hessian_helper_mat;
    ...
    DualVar::init(ColVector<Y_SIZE> y_vec,
                  const SparseRowMatrix& A_mat_rows,
                  const RowMatrix<NUM_CONSTRAINTS, Y_SIZE>& A_sub_rows,
                  const SparseColMatrix& c_vec) {
        ColVector<A_ROWS> slack = (c_vec - A_mat_rows * y_vec).toDense();
        Eigen::Map<ColVector<NUM_CONSTRAINTS>> zero_part(slack.data(), NUM_CONSTRAINTS);
        Eigen::Map<ColMatrix<M_MATRIX_DIM, M_MATRIX_DIM>> matrix_15_15(slack.data() + NUM_CONSTRAINTS, M_MATRIX_DIM, M_MATRIX_DIM);
        for (int i = 0; i < M_MATRIX_DIM; i++)
            matrix_15_15(i, i) += 0.00000001;
        auto matrix_15_15_inv = matrix_15_15.inverse().eval();
        Eigen::Map<ColVector<M_MATRIX_DIM * M_MATRIX_DIM>> matrix_15_15_inv_vec(matrix_15_15_inv.data(),
                                                                                M_MATRIX_DIM * M_MATRIX_DIM);
        auto eigenvals = matrix_15_15.selfadjointView<Eigen::Upper>().eigenvalues();

        return std::make_shared<DualVar>(std::move(y_vec), zero_part, slack, matrix_15_15, matrix_15_15_inv, matrix_15_15_inv_vec, eigenvals, A_sub_rows);
    }

    void DualVar::set_y_vec(ColVector<Y_SIZE> new_y_vec, const SparseRowMatrix& A_mat_rows, const SparseColMatrix& c_vec){
        this->y_vec = std::move(new_y_vec);
        this->slack = (c_vec - A_mat_rows * y_vec).toDense();

        Eigen::Map<ColVector<NUM_CONSTRAINTS>> new_zero_vars(slack.data(), NUM_CONSTRAINTS);
        Eigen::Map<ColMatrix<M_MATRIX_DIM, M_MATRIX_DIM>> new_matrix_15_15(slack.data() + NUM_CONSTRAINTS, M_MATRIX_DIM, M_MATRIX_DIM);
        for (int i = 0; i < M_MATRIX_DIM; i++) {
        ...
        Eigen::Map<ColVector<M_MATRIX_DIM * M_MATRIX_DIM>> new_matrix_15_15_inv_vec(matrix_15_15_inv.data(),
                                                                                M_MATRIX_DIM * M_MATRIX_DIM);
