...
namespace PnP{
    ...
    Eigen::Vector4d PnpProblemSolver::perform_local_search(std::shared_ptr<PnpObjective> pnp_objective, Eigen::Vector4d quats) {
        const auto eps = 0.0000001;
        auto t = 10.0;
        auto mu = 1.5;


        std::function<double(const QuaternionVector &)> func = [&t, pnp_objective](const QuaternionVector &q) -> double{
            double barrier_func = (1 - q.q_norm_squared) * (1 - q.q_norm_squared);
            return q.obj_func(pnp_objective->b) + t * barrier_func;
        };
        std::function<Eigen::Vector4d(const QuaternionVector &)> grad = [&t, pnp_objective](const QuaternionVector &q) -> Eigen::Vector4d {
            Eigen::Vector4d barrier_grad = -2 * (1-q.q_norm_squared) * q.quaternions;
            return q.obj_grad(pnp_objective->b) + t * barrier_grad;
        };
        std::function<Eigen::Matrix4d(const QuaternionVector &)> hess = [&t, pnp_objective](const QuaternionVector &q) -> Eigen::Matrix4d{
