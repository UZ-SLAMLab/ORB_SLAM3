...
namespace PnP
{
	...
	Eigen::Vector4d PnpProblemSolver::perform_local_search(std::shared_ptr<PnpObjective> pnp_objective,
		Eigen::Vector4d quats)
	{
		...
		std::function<Eigen::Vector4d(const QuaternionVector&)>
			grad = [&t, pnp_objective](const QuaternionVector& q) -> Eigen::Vector4d
		{
		  Eigen::Vector4d barrier_grad = -2 * (1 - q.q_norm_squared) * q.quaternions;
		  return q.obj_grad(pnp_objective->b) + t * barrier_grad;
		};
		std::function<Eigen::Matrix4d(const QuaternionVector&)>
			hess = [&t, pnp_objective](const QuaternionVector& q) -> Eigen::Matrix4d
		{
