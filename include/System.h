...
public:
    ...
    cv::Mat TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point>& vImuMeas = vector<IMU::Point>(), string filename="", const bool introspection_on = false, const cv::Mat &costmap = cv::Mat());

    // With introspection
    cv::Mat TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const bool introspection_on, const cv::Mat &costmap);
