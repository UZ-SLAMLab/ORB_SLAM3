...
class CameraClient {
  public:
   CameraClient(std::shared_ptr<Channel> channel)
      : client(CameraService::NewStub(channel)){}

   cv::Mat GetMonoFrame(const std::string& name){
      cv::Mat im,rawData;
      GetFrameRequest requestGet;
      GetFrameResponse responseGet;
      ClientContext context;
      requestGet.set_mime_type("image/jpeg");
      requestGet.set_name(name);

      Status gStatus = client->GetFrame(&context,requestGet,&responseGet);  //render_frame(&context2, request2, &response2);
      if (!gStatus.ok()) {
        std::cout << "Status rpc failed." << gStatus.error_code() <<  std::endl;
        return im;
      }
   ...
   private:
   std::unique_ptr<CameraService::Stub> client;
};
...
cv::Mat im,depth, rawData;
    ...
    ORB_SLAM3::System SLAM("./Vocabulary/ORBvoc.txt",argv[2],ORB_SLAM3::System::MONOCULAR, true, 0, file_nameTraj);
    ...
    while (!SLAM.isShutDown() && b_continue_session)
    {
      ...
      im = camera.GetMonoFrame(cameraName);
...
// cout << "yo got frames" << endl;
        ...
        slamDuration = std::chrono::duration_cast<std::chrono::duration<double> >(timeSLAM - timeBegin).count();
